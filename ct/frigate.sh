...
# --- start CT & set root password --------------------------------------------

pct start "$CTID"

sleep 5

read -s -p "Enter the root password for the LXC: " ROOT_PASSWORD

echo

echo "root:$ROOT_PASSWORD" | pct exec "$CTID" -- chpasswd

# --- gather group IDs inside it ----------------------------------------------
...
sleep 10  # wait for logs to be written

PASSWORD=$(pct exec "$CTID" -- docker logs frigate | grep -m 1 -i "password" | sed 's/.*password: //')

echo -e "${INFO}${YW} Frigate UI: http://${CT_IP}:8971 ${CL}"

echo -e "${INFO}${YW} Frigate Username: admin ${CL}"

echo -e "${INFO}${YW} Frigate Password: $PASSWORD ${CL}"
